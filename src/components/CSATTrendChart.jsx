...
const CSATTrendChart = ({ filters, onDateClick }) => {
        ...
        if (active && payload && payload.length) {
            ...
            return (
                <div style={{
                    ...
                    {payload.map((p, i) => (
                        <p key={i} style={{ margin: '4px 0 0', color: p.color || '#2563EB', fontWeight: '700', fontSize: '0.9rem' }}>
                            {p.name}: {p.value}
                        </p>
                    ))}
                </div>
    ...
    const CustomDot = (props) => {
        ...
        return (
            <g>
                <circle cx={cx} cy={cy} r={3.5} fill="#2563EB" stroke="#0D1117" strokeWidth={2} />
                {showLabel && (
                    <text x={cx} y={cy - 14} textAnchor="middle" fill="#58A6FF" fontSize="10" fontWeight="600">
                        {payload.avg_rating}
                    </text>
                )}
            </g>
                ...
                {loading ? (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <AreaChart
                            ...
                            <Tooltip content={<CustomTooltip />} cursor={{ stroke: '#2563EB', strokeWidth: 1, strokeDasharray: '5 5', fill: 'transparent' }} />
            ...
            {dataWithTrend.length > 0 && (
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <div style={{ width: 12, height: 12, borderRadius: '50%', backgroundColor: '#2563EB' }} />
