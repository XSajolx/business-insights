...
const BAR_COLORS = ['#38BDF8', '#6366F1', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6', '#EC4899', '#06B6D4', '#84CC16', '#F43F5E', '#A78BFA', '#14B8A6', '#FB923C', '#0EA5E9', '#22C55E'];
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flex: '1 1 auto', justifyContent: 'flex-end' }}>
            ...
            <button onClick={exportCSV} style={{
              ...
              borderRadius: '8px', color: '#38BDF8', padding: '6px 14px', fontSize: '0.8rem',
    ...
    const loadEmailData = async () => {
      ...
      try {
        ...
        const sentimentDist = [
          ...
          { name: 'Neutral', value: sentimentMap.Neutral, color: '#6366F1' },
                            ...
                            <Tooltip cursor={{ fill: 'transparent' }} content={({ active, payload }) => {
                              ...
                              return (
                                <div style={{ background: '#1C2128', border: '1px solid #30363D', borderRadius: '8px', padding: '10px 14px', color: '#F0F6FC' }}>
                                  ...
                                  <div style={{ color: '#58A6FF', fontWeight: 700, fontSize: '1rem' }}>{finCountryMetric === 'resolved' ? val : `${val}%`}</div>
                                </div>
                              );
                            }} />
        ...
        <ChartCard title="Knock Count Timeseries" style={{ marginBottom: '1.5rem' }} isLoading={isLoading}>
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={knockCountData}>
              ...
              <Tooltip cursor={{ fill: 'transparent' }} contentStyle={{ background: '#1C2128', border: '1px solid #30363D', borderRadius: '8px' }} labelStyle={{ color: '#F0F6FC', fontWeight: 700 }} itemStyle={{ color: '#F0F6FC' }} />
              <Legend formatter={(value) => <span style={{ color: '#94A3B8', fontSize: '0.8rem' }}>{value}</span>} />
              <Area type="monotone" dataKey="total" stroke="#38BDF8" fill="url(#colorTotal)" strokeWidth={2} name="Total" activeDot={{ r: 6, style: { cursor: 'pointer' }, onClick: (_, payload) => { const date = payload?.payload?.date; if (date) openBarDrillIn(`Total Knock Count — ${date}`, 'conversations', 'Conversations', { mode: 'all-metrics', whereDate: date }); } }} />
        ...
        >
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={performanceData}>
              ...
              <Line type="monotone" dataKey={performanceMetric} stroke="#38BDF8" strokeWidth={2} dot={{ fill: '#38BDF8', r: 3, style: { cursor: 'pointer' } }} activeDot={{ r: 6, style: { cursor: 'pointer' }, onClick: (_, payload) => { const date = payload?.payload?.date; if (!date) return; const label = performanceDropdown.find(d => d.value === performanceMetric)?.label || performanceMetric; const ptMode = performanceMetric === 'conversations' ? 'agent-all-metrics' : 'agent-metric'; openBarDrillIn(`Performance Trend — ${date} (${label})`, performanceMetric, label, { whereDate: date, granularity: timeseriesMode, mode: ptMode }); } }} />
            </LineChart>
          </ResponsiveContainer>
        </ChartCard>
              ...
              <ChartCard title="Email Inflow" isLoading={emailLoading}>
                <ResponsiveContainer width="100%" height={250}>
                  <AreaChart data={emailData?.inflowTrend || []}>
                    ...
                    <Line type="monotone" dataKey="emails" stroke="#38BDF8" strokeWidth={2} dot={false} name="Emails" />
                  </AreaChart>
                </ResponsiveContainer>
              </ChartCard>
